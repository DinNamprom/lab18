...
int main(){
	...
	while(getline(student_file,textline)){
		...
		char tempname[99];
		sscanf(textline.c_str(),"%99[^,],%d,%c,%lf",tempname,&s.id,&s.gender,&s.gpa);
		s.name = tempname;
	...
	while(getline(course_file,textline)){
		if(state == 1){
			...
			if(textline == "---------------------------------------"){
				allcourses.push_back(c);
				c.lecture_list.clear();
				c.student_list.clear();
				state = 1;
			}else{
            	student* p = findstudent(allstudents, stoi(textline));  // Find student by ID
